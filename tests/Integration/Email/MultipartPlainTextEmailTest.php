...
class MultipartPlainTextEmailTest extends ShieldIntegrationTestCase {
	...
	public function test_alert_report_email_transport_generates_alert_only_alt_body() :void {
		...
		], 'Alert report transport alt body' );
		$this->assertTokensAppearInOrder( $plain, [
			'New Issues',
			'Malware Scan',
			'Still Outstanding',
		...
		$this->assertStringNotContainsString( 'Statistics', $plain );
		$this->assertStringNotContainsString( 'Changes', $plain );
	}
	...
	public function test_info_report_email_transport_generates_status_headline_and_details() :void {
		...
		$this->assertContainsTokens( $plain, [
			...
			'Statistics',
			'Changes',
		], 'Info report transport alt body' );
	}
	...
	public function test_back_to_back_sends_do_not_reuse_previous_alt_body() :void {
		...
		$this->assertStringContainsString( 'Alert Status', (string)$mails[ 1 ][ 'alt_body' ] );
	}
}
