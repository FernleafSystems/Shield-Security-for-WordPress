...
class HandleUpgradeIntegrationTest extends ShieldIntegrationTestCase {
	...
	public function testScheduledWorkerWidensScanMetaAndPreservesLargeEligibilityPayloads() :void {
		$this->runWithSeededCronPreservationCheck(
			$this->requireController()->prefix( 'integration-test-handle-upgrade-cron-owner' ),
			function () :void {
				...
				$this->runWithPersistentDatabaseMutation(
					function () use ( $con, $table, $hook, $wpdb, &$committedScanIDs, &$cronSnapshot ) :void {
					...
					function () use ( $con, $table, $targetDefinition, $previousVersion, $previousPersistRequired, $hook, $wpdb, &$committedScanIDs, $configStoreKey, $configStoreSnapshot, &$cronSnapshot ) :void {
