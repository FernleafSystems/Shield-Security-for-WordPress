...
class DatabaseSchemaConfigTest extends TestCase {
	...
	public function test_custom_database_column_names_do_not_use_mysql_reserved_words() :void {
		...
		foreach ( $config[ 'tables' ] ?? [] as $tableKey => $table ) {
			foreach ( \array_keys( $table[ 'cols_custom' ] ?? [] ) as $columnName ) {
				if ( isset( $reservedWords[ \strtolower( (string)$columnName ) ] ) ) {
					$violations[] = sprintf( '%s.%s', $tableKey, $columnName );
				}
			}
		}
