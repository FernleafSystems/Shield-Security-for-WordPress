...
| OM-711 | ~~Add/adjust unit coverage for new breadcrumb and empty-state contracts~~ | `tests/Unit/Utilities/Navigation/BuildBreadCrumbsOperatorModesTest.php`, updated page behavior tests under `tests/Unit/ActionRouter/Render/` | OM-703, OM-708, OM-710 | **Done (2026-03-01)**  -  route-matrix coverage was expanded for mode landing breadcrumbs and investigation page behavior tests now assert non-empty table boot contracts plus empty-state no-table/no-AJAX contract removal (`datatables_init`, `table_action`, and table identity keys). Focused and broad unit suites passed, including full `composer test:unit`. |
