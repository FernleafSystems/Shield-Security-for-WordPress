...
class BackupsCon {
	...
	public function beginBackup() :void {
		$this->status[ 'last_archive_begin_at' ] = Services::Request()->ts();
		$this->store();
	}

	public function endBackup( bool $success ) :void {
		$this->status[ 'last_archive_end_at' ]
			= $this->status[ $success ? 'last_archive_success_at' : 'last_archive_fail_at' ]
			= Services::Request()->ts();
		$this->status[ 'last_archive_duration' ] = Services::Request()->ts() - $this->status[ 'last_archive_begin_at' ];
		$this->store();
	}
	...
	private function resolveStatus() :void {
		if ( $this->isBackupRunning()
			...
			$this->status[ 'last_archive_aborted' ] = true;
			$this->store();
		}
	}
