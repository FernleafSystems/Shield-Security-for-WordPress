...
class PageReports extends BasePluginAdminPage {
	...
	public function renderSectionCustomChart() :string {
		return $this->primary_mod->renderTemplate( '/wpadmin_pages/insights/reports/charts_custom.twig', [
			'ajax'    => [
				'render_custom_chart' => ActionData::BuildJson( Actions\ReportingChartCustom::SLUG ),
			],
			'strings' => [
	...
	private function renderSectionSummaryStats() :string {
		...
		return $this->primary_mod->renderTemplate( '/wpadmin_pages/insights/reports/charts_summary.twig', [
			'ajax' => [
				'render_summary_chart' => ActionData::BuildJson( Actions\ReportingChartSummary::SLUG ),
			],
			'vars' => [
				'stats' => $statsData,
			],
