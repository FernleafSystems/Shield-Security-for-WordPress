...
namespace FernleafSystems\Wordpress\Plugin\Shield\Components\CompCons\SilentCaptcha\Diagnostics;
...
class TestNotBotLoading {
	...
	public function testInternally() :bool {
		$urlToFind = \explode( '?', self::con()->urls->forDistJS( 'silentcaptcha' ) )[ 0 ];
		return \preg_match(
	...
	public function testViaShieldNet() :bool {
		return ( new DetectNotBot() )->run( \explode( '?', self::con()->urls->forDistJS( 'silentcaptcha' ) )[ 0 ] );
	}
}
