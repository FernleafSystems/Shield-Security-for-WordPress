...
if [ -n "${STRAUSS_FORK_REPO:-}" ]; then
  ...
  export STRAUSS_FORK_BRANCH="${STRAUSS_FORK_BRANCH:-develop}"
  export SHIELD_STRAUSS_FORK_BRANCH="${SHIELD_STRAUSS_FORK_BRANCH:-${STRAUSS_FORK_BRANCH}}"
  echo "Using Strauss fork: ${STRAUSS_FORK_REPO} (${SHIELD_STRAUSS_FORK_BRANCH})"
else
